...
export class RegisterComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.registerFormGroup = this.formBuilder.group({
      user: this.formBuilder.group({
        ...
        roles: new FormControl('Select Roles', [
          Validators.required,
        ]),
  ...
  get roles() {
    return this.registerFormGroup.get('user.roles');
  }
  ...
  onSubmit() {
    ...
    user.append('roles', this.roles?.value);
    ...
    this.authenticationService.signup(user).subscribe({
      next: () => {
        ...
        this.loading = false;
        this.router.navigate([`/users/${this.authenticationService.getAuthenticatedUserId()}`])
      },
      error: () => {
